 ...
 * Generated: 2026-01-31T22:12:13.413Z
 * 
...
class Premium;
class Discount;
struct PreRangeLevels;
class CSession;
struct sessiondata;
struct sessiontime;
struct SETTINGS;
struct SimTradingConfig;
class CSimulatedTrading;
struct sockaddr_in;
struct WSAData;
class CString;
struct SwingConfig;
...
class CTerminalInfo;
class CTrade;
class BandOsMaSignal;
class SimpleStrategy;
struct TesterRecord;
struct PairOfPeriods;
struct PhaseConfig;
class CTradingPhaseManager;
class CWebsocket;
...
enum ENUM_MacroWindow {
    MACRO_NONE = 0,
    MACRO_SILVERBULLET = 1,
    MACRO_FEIERABEND = 2
};

enum ENUM_TradingPhase {
    PHASE_WAITING = 0,
    PHASE_MACRO_ACTIVE = 1,
    PHASE_TRADING = 2,
    PHASE_CLOSED = 3
};
...
enum ENUM_ActivationState {
    ACTIVATION_WAITING = 0,
    ACTIVATION_INSIDE = 1,
    ACTIVATION_EXITED = 2
};

enum ENUM_ENTRY_FVG_STATE {
    ENTRY_FVG_NONE,
    ENTRY_FVG_WAITING_TOUCH,
    ENTRY_FVG_TOUCHED,
    ENTRY_FVG_CONFIRMED,
    ENTRY_FVG_INVALIDATED
};
...
enum ObjectSyncCommand {
    OBJ_SYNC_CREATE = 1,
    OBJ_SYNC_MODIFY = 2,
    OBJ_SYNC_DELETE = 3,
    OBJ_SYNC_MOVE = 4,
    OBJ_SYNC_CREATE_COMPOSITE = 5
};
...
enum ENUM_TargetMode {
    RMultiple,
    LiquidityObject
};

enum ENUM_LiquidityObjectType {
    LIQUIDITY_LINE,
    EQUAL_LIQUIDITY
};

enum ENUM_TargetSelectionPolicy {
    NearestPrice,
    MostRecent
};
...
enum ENUM_BreakEvenMode {
    Disabled,
    AtRMultiple,
    AtCustom
};

enum ENUM_PremiumDiscountSource {
    DealingRangeMid,
    SessionRangeMid,
    Swing50
};
...
enum ENUM_TriggerPolicy {
    AND,
    OR
};

enum ENUM_ConfirmationRecencyPolicy {
    None,
    WithinNBars,
    WithinMinutes
};
...
public:
    int index;
    double isLiquidChart;
    CCandle();
    CCandle(datetime time);
    CCandle(int barIndex);
    ~CCandle();
    void useLiquidChart(double & OpenLC, double & HighLC, double & LowLC, double & CloseLC);
    bool isBullish();
    bool isBearish();
    bool isBullishNext();
    bool isBullishPrevious();
    bool isBearishNext();
    bool isBearishPrevious();
    bool isFVG();
...
public:
    ...
    int lookbackBars;
    int maxDateSearchAttempts;
    int dateSearchStep;
    NavConfig(int threshold = 240, int lookback = 240, int maxAttempts = 10, int step = 3600);
};

class CChartNavigation {
public:
    CChartNavigation();
    CChartNavigation(const NavConfig& config);
    void SetupChart(long chartID = 0);
    bool IsNearRightEdge(long chartID = 0);
    bool NavigateToDate(datetime targetDate, bool isForward, long chartID = 0, string logPrefix = "");
    void AutoScaleToShift(int shiftTarget, bool isForward, long chartID = 0);
    void AutoScaleVisibleBars(long chartID = 0);
    bool NavigateToRecent(long chartID = 0);
    datetime CalculateBackwardTarget(datetime referenceDate);
    datetime CalculateForwardTarget(datetime referenceDate);
    datetime GetNavigationReferenceDate(long chartID = 0);
    datetime NavigateBackward(long chartID = 0);
    datetime NavigateForward(long chartID = 0);
    void SetConfig(const NavConfig & config);
};
...
class CFVGManager {
public:
    CFVGManager(double slBufferPoints = 1.0);
    bool DetectFVGOnBar(int barIndex, FVGData & fvg);
    bool IsFVGValidForBias(ENUM_FVGType type, ENUM_Bias bias);
    bool CheckInversion(FVGData & fvg, double prevClose);
    bool CheckActivation(FVGData & fvg, double prevClose);
    bool CheckInvalidation(FVGData & fvg, double prevClose);
    int FindBestEntryFVG(FVGData & activeFVGs, ENUM_Bias bias, double currentPrice);
    bool AddToArray(FVGData & fvg, FVGData & array);
    bool DoFVGsOverlap(const FVGData & fvg1, const FVGData & fvg2);
    int FindClosestReachableFVG(FVGData & activeFVGs, ENUM_Bias bias, double currentPrice);
};
...
public:
    ...
    bool active;
    ENUM_FVGType type;
    ENUM_FVGType originalType;
    datetime time;
    double upperEdge;
    double lowerEdge;
    double entryPrice;
    double stopLoss;
    ENUM_MacroWindow macroSource;
    void Reset();
};
...
class CLinMin {
public:
    static double m_ftol;
    static double m_xtol;
    static int m_maxfev;
    static double m_stpmin;
    static double m_defstpmax;
    static double m_armijofactor;
...
class CFtBase {
public:
    static int m_coltype;
    static int m_coloperandscnt;
    static int m_coloperandsize;
    static int m_colmicrovectorsize;
    static int m_colparam0;
    static int m_colparam1;
    static int m_colparam2;
    static int m_colparam3;
    static int m_colscnt;
    static int m_opend;
    static int m_opcomplexreffft;
    static int m_opbluesteinsfft;
    static int m_opcomplexcodeletfft;
    static int m_opcomplexcodelettwfft;
    static int m_opradersfft;
    static int m_opcomplextranspose;
    static int m_opcomplexfftfactors;
    static int m_opstart;
    static int m_opjmp;
    static int m_opparallelcall;
    static int m_MaxRadix;
    static int m_updatetw;
    static int m_recursivethreshold;
    static int m_raderthreshold;
    static int m_ftbasecodeletrecommended;
    static double m_ftbaseinefficiencyfactor;
    static int m_ftbasemaxsmoothfactor;
...
class CNearestNeighbor {
public:
    static int m_splitnodesize;
    static int m_kdtreefirstversion;
...
class CHighQualityRand {
public:
    static int m_HQRndMax;
    static int m_HQRndM1;
    static int m_HQRndM2;
    static int m_HQRndMagic;
...
public:
    ...
    static double m_machineepsilon;
    static double m_maxrealnumber;
    static double m_minrealnumber;
...
class CDForest {
public:
    static int m_InnerNodeWidth;
    static int m_LeafNodeWdth;
    static int m_DFUseStrongSplits;
    static int m_DFUseEVS;
    static int m_DFUncompressedV0;
    static int m_DFCompressedV0;
    static int m_NeedTrnGini;
    static int m_NeedOOBGini;
    static int m_NeedPermutation;
    static int m_PermutationImportanceBatchSize;
...
class CMLPBase {
public:
    static int m_mlpvnum;
    static int m_mlpfirstversion;
    static int m_nfieldwidth;
    static int m_hlconm_nfieldwidth;
    static int m_hlm_nfieldwidth;
    static int m_gradbasecasecost;
    static int m_microbatchsize;
...
public:
    static double m_xtol;
    static double m_ftol;
    static double m_gtol;
    static int m_maxfev;
    static double m_stpmin;
    static double m_stpmax;
    static int m_logitvnum;
...
class CMLPTrain {
public:
    static double m_mindecay;
    static int m_defaultlbfgsfactor;
...
class CClustering {
public:
    static int m_kmeansblocksize;
    static int m_kmeansparalleldim;
    static int m_kmeansparallelk;
    static double m_complexitymultiplier;
    static void ClusterizerCreate(CClusterizerState & s);
    static void ClusterizerSetPoints(CClusterizerState & s, CMatrixDouble & xy, int npoints, int nfeatures, int disttype);
    static void ClusterizerSetDistances(CClusterizerState & s, CMatrixDouble & d, int npoints, bool IsUpper);
    static void ClusterizerSetAHCAlgo(CClusterizerState & s, int algo);
...
class CODESolver {
public:
    static double m_odesolvermaxgrow;
    static double m_odesolvermaxshrink;
...
public:
    static double m_w0;
    static double m_meps;
    static int m_defaultnlayers;
    static double m_defaultlambda0;
...
class CSpline1D {
public:
    static double m_lambdareg;
    static double m_cholreg;
...
public:
    static double m_cholreg;
    static double m_lambdaregblocklls;
    static double m_lambdaregfastddm;
    static double m_lambdadecay;
...
public:
    static int m_mxnx;
    static double m_rbffarradius;
    static double m_rbfnearradius;
    static double m_rbfmlradius;
    static double m_minbasecasecost;
...
class CRBFV2 {
public:
    static double m_defaultlambdareg;
    static double m_defaultsupportr;
    static int m_defaultmaxits;
    static int m_defaultbf;
    static int m_maxnodesize;
    static double m_complexitymultiplier;
...
class CRBFV3 {
public:
    static double m_epsred;
    static int m_maxddmits;
    static double m_polyharmonic2scale;
    static int m_acbfparallelthreshold;
    static int m_ddmparallelthreshold;
    static int m_bfparallelthreshold;
...
public:
    static double m_eps;
    static double m_rbffarradius;
    static int m_rbffirstversion;
    static int m_rbfversion2;
    static int m_rbfversion3;
    static void RBFCreate(int nx, int ny, CRBFModel& s);
    static void RBFCreateCalcBuffer(CRBFModel& s, CRBFCalcBuffer& buf);
    static void RBFSetPoints(CRBFModel& s, CMatrixDouble& xy, int n);
    static void RBFSetPointsAndScales(CRBFModel& r, CMatrixDouble& xy, int n, CRowDouble& s);
...
class CAblas {
public:
    static int m_blas2minvendorkernelsize;
...
class CSparse {
public:
    static double m_DesiredLoadFactor;
    static double m_MaxLoadFactor;
    static double m_GrowFactor;
    static int m_Additional;
    static int m_LinAlgSwitch;
...
class CSpTrf {
public:
    static double m_DenseBnd;
    static int m_SlsWidth;
    static bool SpTrfLU(CSparseMatrix & a, int pivottype, CRowInt & pr, CRowInt & pc, CSLUV2Buffer & buf);
};

class CAmdOrdering {
public:
    static int m_KNsHeaderSize;
    static int m_LLMentrySize;
...
class CSpChol {
public:
    static int m_MaxSupernode;
    static double m_MaxMergeinEfficiency;
    static int m_SmallFakesTolerance;
    static int m_MaxFastKernel;
    static bool m_RelaxedSupernodes;
...
class COptServ {
public:
    static double m_ognoiselevelf;
    static double m_ognoiselevelg;
    static double m_ogminrating0;
    static double m_ogminrating1;
...
class CSActiveSets {
public:
    static int m_maxbasisage;
    static double m_maxbasisdecay;
    static double m_minnormseparation;
...
class CMinBLEIC {
public:
    static double m_maxnonmonotoniclen;
    static double m_gtol;
    static double m_initialdecay;
    static double m_mindecay;
    static double m_decaycorrection;
    static double m_penaltyfactor;
...
class CVIPMSolver {
public:
    static double m_muquasidense;
    static int m_maxipmits;
    static double m_initslackval;
    static double m_steplengthdecay;
    static double m_stagnationdelta;
    static double m_primalinfeasible1;
    static double m_dualinfeasible1;
    static double m_bigy;
    static double m_ygrowth;
    static int m_itersfortoostringentcond;
    static int m_minitersbeforedroppingbounds;
    static int m_minitersbeforeinfeasible;
    static int m_minitersbeforestagnation;
    static int m_minitersbeforeeworststagnation;
    static int m_primalstagnationlen;
    static int m_dualstagnationlen;
    static double m_bigconstrxtol;
    static double m_bigconstrmag;
    static double m_minitersbeforesafeguards;
    static double m_badsteplength;
...
class CCQModels {
public:
    static int m_newtonrefinementits;
...
class CQQPSolver {
public:
    static int m_quickqprestartcg;
    static double m_regz;
...
class CQPDenseAULSolver {
public:
    static double m_evictionlevel;
    static double m_expansionratio;
...
class CMinLM {
public:
    static double m_lambdaup;
    static double m_lambdadown;
    static double m_suspiciousnu;
    static int m_smallmodelage;
    static int m_additers;
...
public:
    ...
    static double m_stpmin;
    static double m_gtol;
    static double m_gpaftol;
    static double m_gpadecay;
    static double m_asarho;
...
class CNLCSQP {
public:
    static int m_fstagnationlimit;
    static int m_penaltymemlen;
    static int m_trustradstagnationlimit;
    static double m_augmentationfactor;
    static double m_inittrustrad;
    static double m_maxbigc;
    static double m_maxtrustraddecay;
    static double m_maxtrustradgrowth;
    static double m_meritfunctionbase;
    static double m_meritfunctiongain;
    static double m_sqpbigscale;
    static double m_sqpdeltadecrease;
    static double m_sqpdeltaincrease;
    static double m_sqpsmallscale;
    static double m_stagnationepsf;
...
class CRevisedDualSimplex {
public:
    static int m_maxforcedrestarts;
    static int m_safetrfage;
    static int m_defaultmaxtrfage;
    static double m_minbeta;
    static double m_maxudecay;
    static double m_shiftlen;
    static double m_alphatrigger;
    static double m_alphatrigger2;
    static int m_ssinvalid;
    static int m_ssvalidxn;
    static int m_ssvalid;
    static int m_ccfixed;
    static int m_cclower;
    static int m_ccupper;
    static int m_ccrange;
    static int m_ccfree;
    static int m_ccinfeasible;
...
class CNLCSLP {
public:
    static double m_slpstpclosetozero;
    static double m_slpdeltadecrease;
    static double m_slpdeltaincrease;
    static double m_slpstpclosetoone;
    static double m_maxtrustraddecay;
    static double m_maxtrustradgrowth;
    static double m_slpgtol;
    static double m_initbigc;
    static double m_maxbigc;
    static double m_bfgstol;
    static double m_defaultl1penalty;
    static double m_meritfunctionbase;
    static double m_meritfunctiongain;
    static double m_inequalitydampingfactor;
    static double m_augmentationfactor;
    static double m_inittrustrad;
    static double m_stagnationepsf;
    static int m_lpfailureslimit;
    static int m_fstagnationlimit;
    static int m_nondescentlimit;
    static int m_nonmonotonicphase2limit;
    static double m_slpbigscale;
    static double m_slpsmallscale;
    static double m_defaultmaglagdecay;
...
class CMinNLC {
public:
    static int m_lbfgsfactor;
    static double m_aulmaxgrowth;
    static double m_maxlagmult;
    static double m_hessesttol;
    static double m_initgamma;
    static double m_regprec;
    static void MinNLCCreate(int n, CRowDouble & x, CMinNLCState & State);
    static void MinNLCCreateF(int n, CRowDouble & x, double diffstep, CMinNLCState & State);
    static void MinNLCSetBC(CMinNLCState & State, CRowDouble & bndl, CRowDouble & bndu);
    static void MinNLCSetLC(CMinNLCState & State, CMatrixDouble & c, CRowInt & ct, int k);
...
class CLinCG {
public:
    static double m_defaultprecision;
    static void LinCGCreate(int n, CLinCGState & state);
    static void LinCGSetStartingPoint(CLinCGState & state, CRowDouble & x);
...
class CLinLSQR {
public:
    static double m_atol;
    static double m_btol;
    static void LinLSQRCreate(int m, int n, CLinLSQRState & state);
    static void LinLSQRCreateBuf(int m, int n, CLinLSQRState & state);
...
public:
    ...
    static bool Run(const string ExpertName = NULL, const string Symb = NULL, const ENUM_TIMEFRAMES period = PERIOD_CURRENT, const datetime iBeginTime = 0, const datetime iEndTime = 0);
...
public:
    ...
    datetime entrytime;
    datetime exittime;
    void init(string pSymbol, string pEntrytimeframe);
    int durationMin();
    bool parametersComplete();
    bool isShort();
    bool isLong();
    bool isTarget(double target);
    string direction();
};
...
public:
    string name;
    string subwindowString;
    string periodString;
    string type;
    string candleTime;
    void read(string obj_name);
};
...
public:
    ...
    long timeframes;
    int fiboLevels;
    string fiboLevelText;
    double fiboLevelValue;
...
class PDarray {
public:
    PDarray(string n, ModusType t);
    virtual string GetName();
    virtual ModusType GetType();
    virtual string GetClassName();
    virtual bool IsInBalancedPriceRange();
protected:
    string obj_name;
    ModusType type;
    double entryPrice;
    double firstQuadrant;
    double mean;
    double thirdQuadrant;
    double c_e_;
    int timesUsed;
};

class Premium : public PDarray {
public:
    Premium(string n, ModusType t);
    virtual string GetClassName();
};

class Discount : public PDarray {
public:
    Discount(string n, ModusType t);
    virtual string GetClassName();
};
...
public:
    ...
    sessiondata london;
    sessiondata newyork;
    sessiondata newyorkPM;
    sessiondata lasthour;
    static sessiontime siatime;
    static sessiontime londontime;
    static sessiontime newyorktime;
    static sessiontime lasthourtime;
    CSession();
    ~CSession();
    void setCurrentTime();
    string current();
    ...
    bool isNewYork();
    bool isLastHour();
    string getName(MqlDateTime & candleTime);
    void setAsia(int starthour, int starminute, int endhour, int endminute, color colorSession);
    void setLondon(int starthour, int starminute, int endhour, int endminute, color colorSession);
    void setNewYork(int starthour, int starminute, int endhour, int endminute, color colorSession);
    void setNewYorkPM(int starthour, int starminute, int endhour, int endminute, color colorSession);
    void setLastHour(int starthour, int starminute, int endhour, int endminute, color colorSession);
    void createAll();
    void update();
    void createSession(int indexBar, MqlDateTime & indexMdt, int sessionShiftMin, sessiondata & data, bool isEnd = false, bool selected = false);
    void createTrueDayOpen(int indexBar, MqlDateTime & indexMdt);
    void createEquitiesOpen(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createPreRangeOpen(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createSilverBullet(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNypm(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createPreRangeLiquidityLines(datetime equitiesOpenTime, int equitiesOpenBar);
    void checkPreRangeLiqCrosses();
    void createLNQ2(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createLNQ4(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYQ1(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYQ2(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYQ3(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYQ4(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYpmQ1(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYpmQ2(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYpmQ3(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createNYpmQ4(int indexBar, MqlDateTime & indexMdt, sessiondata & data);
    void createSessionLevels(sessiondata & data, datetime pDate);
protected:
    void setSessiondata(sessiondata & data, int starthour, int startminute, int endhour, int endminute, color colorSession);
    void calcMean(sessiondata & data);
    void calcExpansions(sessiondata & data, double multiplier1, double multiplier2, double multiplier3);
};
...
public:
    ...
    MqlDateTime start;
    MqlDateTime end;
    MqlDateTime equitiesOpen;
    MqlDateTime preRangeOpen;
    MqlDateTime q1;
    MqlDateTime q2;
    MqlDateTime q3;
    MqlDateTime q4;
    ...
    double mean;
    double low;
    double bullishExpansion1;
    double bullishExpansion2;
    double bullishExpansion3;
    double bearishExpansion1;
    double bearishExpansion2;
    double bearishExpansion3;
};
...
public:
    ...
    datetime BeginTime;
    datetime EndTime;
    void Set(const string sExpertName = NULL, const string sSymb = NULL, const ENUM_TIMEFRAMES ePeriod = PERIOD_CURRENT, const datetime dBeginTime = 0, const datetime dEndTime = 0);
...
public:
    ...
    double tickSize;
    string symbol;
    SimTradingConfig(double comm = 10.0, double tickVal = 5.0, double tickSz = 0.25, string sym = "");
};

class CSimulatedTrading {
public:
    CSimulatedTrading(const SimTradingConfig & config = SimTradingConfig());
    void Reset();
    bool PlaceLimitOrder(ENUM_ORDER_TYPE type, double entryPrice, double stopLoss, double takeProfit, double lotSize, int currentBarIndex);
    bool CancelOrder();
    bool CheckOrderFill(double bid, double ask, int currentBarIndex);
    bool CheckPositionExit(double bid, double ask, int currentBarIndex, string & exitReason);
    bool ClosePositionAtMarket(double bid, double ask, string reason);
    bool HasOrder() const;
    bool HasPosition() const;
    bool IsLong() const;
    SimulatedOrder& GetOrder() const;
    SimulatedPosition& GetPosition() const;
    SimulatedTradeSummary& GetSummary() const;
    double GetPositionPnL(double currentPrice) const;
};
...
public:
    int fractalBars;
    int lookbackDays;
    SwingConfig(int fractal = 2, int lookback = 2);
};

class CSwingDetector {
public:
    CSwingDetector();
    CSwingDetector(const SwingConfig& config);
    bool FindSwingHigh(int startBar, int endBar, double & swingHigh, int & swingBar);
    bool FindSwingLow(int startBar, int endBar, double & swingLow, int & swingBar);
    bool FindNearestSwingHighAbove(double currentPrice, datetime startTime, double & swingHigh);
    bool FindNearestSwingLowBelow(double currentPrice, datetime startTime, double & swingLow);
    void SetConfig(const SwingConfig & config);
};
...
public:
    ...
    bool Add(const string ExpertName = NULL, const string Symb = NULL, const ENUM_TIMEFRAMES period = PERIOD_CURRENT, const datetime BeginTime = 0, const datetime EndTime = 0, const INITDEINIT fInit = NULL, const INITDEINIT fDeinit = NULL);
...
public:
    BandOsMaSignal(const int fast, const int slow, const int signal, const ENUM_APPLIED_PRICE price, const int bands, const int shift, const double deviation, const int period, const int x, ENUM_MA_METHOD method);
    ~BandOsMaSignal();
...
class CTradingPhaseManager {
public:
    CTradingPhaseManager(const PhaseConfig & config = PhaseConfig());
    void Reset();
    bool UpdatePhase(datetime currentTime);
    ENUM_TradingPhase GetCurrentPhase() const;
    ENUM_MacroWindow GetCurrentMacro() const;
    ENUM_MacroWindow GetLastCompletedMacro() const;
    bool IsInMacro() const;
    bool IsInSilverBullet() const;
    bool IsInFeierabend() const;
    bool IsTradingClosed() const;
    bool IsTrading() const;
    bool IsWaiting() const;
    bool IsMacroActive() const;
    string GetCurrentMacroName() const;
};
...
public:
    ...
    string lpAssemblyEncodedAssemblyIdentity;
    string lpAssemblyManifestPath;
    string lpAssemblyPolicyPath;
    string lpAssemblyDirectoryName;
    uint ulFileCount;
};
...
public:
    ...
    string lpRootManifestPath;
    string lpRootConfigurationPath;
    string lpAppDirPath;
};
...
class CalendarFilterCached;
class ChartModeMonitorInterface;
class ChartModeMonitorBase;
class ChartModeMonitor;
struct Sum;
class Comments;
class Converter;
class CRC32;
class CustomOrder;
class CustomSymbolMonitor;
class DateTime;
struct DBValue;
...
struct MqlTradeResultSync;
struct MqlTradeRequestSync;
class AsyncSwitcher;
...
class ObjectMonitorInterface;
class ObjectMonitorBase;
class ObjectMonitor;
struct TesterOptCacheHeader;
struct TestCacheInput;
struct TestCacheInputExtended;
struct TestCacheRecord;
struct ExpTradeSummary;
struct Doubles;
struct Integers;
struct ExpTradeSummaryBySymbols;
struct MathTestCacheRecord;
class RecordBase;
...
class TradeBaseState;
class SymbolMetrics;
struct TradeDirection;
class TrailingStop;
class TrailingStopByMA;
struct Tuple2;
struct Tuple3;
struct Tuple4;
struct Tuple5;
...
class WebSocketFrameHixie;
class IWebSocketFrame;
class StaticCreator;
class Creator;
class IWebSocketMessage;
class WebSocketMessageHixie;
class WebSocketMessage;
class WebSocketConnection;
class WebSocketConnectionHybi;
class WebSocketConnectionHixie;
struct HixieKey;
class MqlWebSocketTransport;

// Enums
enum APPLIED_TO {
    APPLIED_TO_DEFAULT_CLOSE_NA,
    APPLIED_TO_CLOSE,
    APPLIED_TO_OPEN,
    APPLIED_TO_HIGH,
    APPLIED_TO_LOW,
    ...
    APPLIED_TO_PREVIOUS,
    APPLIED_TO_FIRST,
    APPLIED_TO_HANDLE_10,
    APPLIED_TO_HANDLE_11,
    APPLIED_TO_HANDLE_12,
    APPLIED_TO_HANDLE_13,
    APPLIED_TO_HANDLE_14,
    APPLIED_TO_HANDLE_15
};
...
enum IndicatorType {
    ...
    1,
    IND_AC,
    iAD_volume = MAKE_IND(1,
    1,
    1,
    IND_AD,
    iADX_period = MAKE_IND(1,
    3,
    1,
    IND_ADX,
    iADXWilder_period = MAKE_IND(1,
    3,
    1,
    IND_ADXW,
    iAlligator_jawP_jawS_teethP_teethS_lipsP_lipsS_method_price = MAKE_IND(8,
    3,
    0,
    IND_ALLIGATOR,
    iAMA_period_fast_slow_shift_price = MAKE_IND(5,
    1,
    0,
    IND_AMA,
    iAO_ = MAKE_IND(0,
    ...
    1,
    IND_AO,
    iATR_period = MAKE_IND(1,
    1,
    1,
    IND_ATR,
    iBands_period_shift_deviation_price = MAKE_IND(4,
    3,
    0,
    IND_BANDS,
    iBearsPower_period = MAKE_IND(1,
    1,
    1,
    IND_BEARS,
    iBullsPower_period = MAKE_IND(1,
    1,
    1,
    IND_BULLS,
    iBWMFI_volume = MAKE_IND(1,
    1,
    1,
    IND_BWMFI,
    iCCI_period_price = MAKE_IND(2,
    1,
    1,
    IND_CCI,
    iChaikin_fast_slow_method_volume = MAKE_IND(4,
    1,
    1,
    IND_CHAIKIN,
    iDEMA_period_shift_price = MAKE_IND(3,
    1,
    0,
    IND_DEMA,
    iDeMarker_period = MAKE_IND(1,
    1,
    1,
    IND_DEMARKER,
    iEnvelopes_period_shift_method_price_deviation = MAKE_IND(5,
    2,
    0,
    IND_ENVELOPES,
    iForce_period_method_volume = MAKE_IND(3,
    1,
    1,
    IND_FORCE,
    iFractals_ = MAKE_IND(0,
    2,
    0,
    IND_FRACTALS,
    iFrAMA_period_shift_price = MAKE_IND(3,
    1,
    0,
    IND_FRAMA,
    iGator_jawP_jawS_teethP_teethS_lipsP_lipsS_method_price = MAKE_IND(8,
    4,
    1,
    IND_GATOR,
    iIchimoku_tenkan_kijun_senkou = MAKE_IND(3,
    5,
    0,
    IND_ICHIMOKU,
    iMomentum_period_price = MAKE_IND(2,
    1,
    1,
    IND_MOMENTUM,
    iMFI_period_volume = MAKE_IND(2,
    1,
    1,
    IND_MFI,
    iMA_period_shift_method_price = MAKE_IND(4,
    1,
    0,
    IND_MA,
    iMACD_fast_slow_signal_price = MAKE_IND(4,
    2,
    1,
    IND_MACD,
    iOBV_volume = MAKE_IND(1,
    1,
    1,
    IND_OBV,
    iOsMA_fast_slow_signal_price = MAKE_IND(4,
    1,
    1,
    IND_OSMA,
    iRSI_period_price = MAKE_IND(2,
    1,
    1,
    IND_RSI,
    iRVI_period = MAKE_IND(1,
    2,
    1,
    IND_RVI,
    iSAR_step_maximum = MAKE_IND(2,
    1,
    0,
    IND_SAR,
    iStdDev_period_shift_method_price = MAKE_IND(4,
    1,
    1,
    IND_STDDEV,
    iStochastic_K_D_slowing_method_price = MAKE_IND(5,
    2,
    1,
    IND_STOCHASTIC,
    iTEMA_period_shift_price = MAKE_IND(3,
    1,
    0,
    IND_TEMA,
    iTriX_period_price = MAKE_IND(2,
    1,
    1,
    IND_TRIX,
    iVIDyA_momentum_smooth_shift_price = MAKE_IND(4,
    1,
    0,
    IND_VIDYA,
    iVolumes_volume = MAKE_IND(1,
    1,
    1,
    IND_VOLUMES,
    iWPR_period = MAKE_IND(1,
    1,
    1,
    IND_WPR
};
...
enum ENUM_CALENDAR_PROPERTY {
    CALENDAR_PROPERTY_COUNTRY_ID,
    CALENDAR_PROPERTY_COUNTRY_NAME,
    CALENDAR_PROPERTY_COUNTRY_CODE,
    CALENDAR_PROPERTY_COUNTRY_CURRENCY,
    CALENDAR_PROPERTY_COUNTRY_GLYPH,
    CALENDAR_PROPERTY_COUNTRY_URL,
    CALENDAR_PROPERTY_EVENT_ID,
    CALENDAR_PROPERTY_EVENT_TYPE,
    CALENDAR_PROPERTY_EVENT_SECTOR,
    CALENDAR_PROPERTY_EVENT_FREQUENCY,
    CALENDAR_PROPERTY_EVENT_TIMEMODE,
    CALENDAR_PROPERTY_EVENT_UNIT,
    CALENDAR_PROPERTY_EVENT_IMPORTANCE,
    CALENDAR_PROPERTY_EVENT_MULTIPLIER,
    CALENDAR_PROPERTY_EVENT_DIGITS,
    CALENDAR_PROPERTY_EVENT_SOURCE,
    CALENDAR_PROPERTY_EVENT_CODE,
    CALENDAR_PROPERTY_EVENT_NAME,
    CALENDAR_PROPERTY_RECORD_ID,
    CALENDAR_PROPERTY_RECORD_TIME,
    CALENDAR_PROPERTY_RECORD_PERIOD,
    CALENDAR_PROPERTY_RECORD_REVISION,
    ...
    CALENDAR_PROPERTY_CHANGE_ID
};

enum ENUM_CALENDAR_EVENT_TYPE_EXT {
    TYPE_EVENT = CALENDAR_TYPE_EVENT,
    TYPE_INDICATOR = CALENDAR_TYPE_INDICATOR,
    TYPE_HOLIDAY = CALENDAR_TYPE_HOLIDAY,
    TYPE_ANY
};

enum ENUM_CALENDAR_EVENT_SECTOR_EXT {
    SECTOR_NONE = CALENDAR_SECTOR_NONE,
    SECTOR_MARKET = CALENDAR_SECTOR_MARKET,
    SECTOR_GDP = CALENDAR_SECTOR_GDP,
    SECTOR_JOBS = CALENDAR_SECTOR_JOBS,
    SECTOR_PRICES = CALENDAR_SECTOR_PRICES,
    SECTOR_MONEY = CALENDAR_SECTOR_MONEY,
    SECTOR_TRADE = CALENDAR_SECTOR_TRADE,
    SECTOR_GOVERNMENT = CALENDAR_SECTOR_GOVERNMENT,
    SECTOR_BUSINESS = CALENDAR_SECTOR_BUSINESS,
    SECTOR_CONSUMER = CALENDAR_SECTOR_CONSUMER,
    SECTOR_HOUSING = CALENDAR_SECTOR_HOUSING,
    SECTOR_TAXES = CALENDAR_SECTOR_TAXES,
    SECTOR_HOLIDAYS = CALENDAR_SECTOR_HOLIDAYS,
    SECTOR_ANY
};

enum ENUM_CALENDAR_EVENT_IMPORTANCE_EXT {
    IMPORTANCE_NONE = CALENDAR_IMPORTANCE_NONE,
    IMPORTANCE_LOW = CALENDAR_IMPORTANCE_LOW,
    IMPORTANCE_MODERATE = CALENDAR_IMPORTANCE_MODERATE,
    IMPORTANCE_HIGH = CALENDAR_IMPORTANCE_HIGH,
    IMPORTANCE_ANY
};
...
enum TRADE_RESTRICTIONS {
    ...
    ACCOUNT_RESTRICTION = 16,
    EXPERTS_RESTRICTION = 32
};
...
enum COPY_TICKS {
    ALL_TICKS = COPY_TICKS_ALL,
    INFO_TICKS = COPY_TICKS_INFO,
    TRADE_TICKS = COPY_TICKS_TRADE
};

enum TICK_FLAGS {
    TF_BID = TICK_FLAG_BID,
    TF_ASK = TICK_FLAG_ASK,
    TF_BID_ASK = TICK_FLAG_BID | TICK_FLAG_ASK,
    TF_LAST = TICK_FLAG_LAST,
    ...
    TF_BUY = TICK_FLAG_BUY,
    TF_SELL = TICK_FLAG_SELL,
...
enum TICK_MODEL {
    TICK_MODEL_UNKNOWN = -1,
    TICK_MODEL_REAL = 0,
    TICK_MODEL_GENERATED = 1,
    TICK_MODEL_OHLC_M1 = 2,
    TICK_MODEL_OPEN_PRICES = 3,
    TICK_MODEL_MATH_CALC = 4,
    Math,
    calculations
};
...
class AutoIndicator {
public:
    AutoIndicator(const IndicatorType t, const string custom, const string parameters, const string s = NULL, const ENUM_TIMEFRAMES p = 0);
    int getHandle() const;
...
public:
    struct MqlCalendarCountryRef;
    struct MqlCalendarEventRef;
    CalendarCache(const string _context = NULL, const datetime _from = 0, const datetime _to = 0);
    ...
    static int ArrayBlowerBound(const ulong & array, const ulong value, const int index);
    int calendarValueHistoryByEvent(ulong event_id, MqlCalendarValue & temp, datetime _from, datetime _to = 0);
    int calendarValueHistory(MqlCalendarValue & temp, datetime _from, datetime _to = 0, const string _code = NULL, const string _coin = NULL);
    int calendarValueLast(ulong & change, MqlCalendarValue & result, const string code = NULL, const string currency = NULL);
    int calendarValueLastByEvent(ulong event_id, ulong & change, MqlCalendarValue & result);
};
...
public:
    CalendarFilterCached(CalendarCache * _cache);
...
protected:
    ...
    virtual int calendarValueHistoryByEvent(ulong event_id, MqlCalendarValue & temp, datetime _from, datetime _to = 0);
    virtual int calendarValueHistory(MqlCalendarValue & temp, datetime _from, datetime _to = 0, const string _code = NULL, const string _coin = NULL);
    virtual int calendarValueLast(ulong & _change, MqlCalendarValue & result, const string _code = NULL, const string _coin = NULL);
...
public:
    DBRowStruct(const int q, const bool c = false);
    S get() const;
protected:
    S object;
    virtual bool DBread();
...
class DBQuery {
public:
    DBQuery(const int owner, const string s);
    ~DBQuery();
    bool isValid() const;
    int getHandle() const;
    int getDB() const;
...
class DBSQLite {
public:
    DBSQLite(const string file, const uint opts = DATABASE_OPEN_CREATE | DATABASE_OPEN_READWRITE);
    ~DBSQLite();
    int getHandle() const;
    bool isOpen() const;
...
class DBTransaction {
public:
    DBTransaction(DBSQLite & owner, const bool c = false);
    ~DBTransaction();
    bool commit();
};
...
public:
    virtual string stringify(const long v, const ENUM_DEAL_PROPERTY_INTEGER property) const;
    virtual string stringify(const ENUM_DEAL_PROPERTY_INTEGER property) const;
    virtual string stringify(const ENUM_DEAL_PROPERTY_DOUBLE property, const string format = NULL) const;
    virtual string stringify(const ENUM_DEAL_PROPERTY_STRING property) const;
    virtual string stringify(const int i) const;
};

class FileHandle {
public:
    FileHandle(const int h = INVALID_HANDLE);
    FileHandle(int & holder, const int h);
    ~FileHandle();
    void close();
};
...
public:
    ...
    int HEAD(const string address, uchar & result, string & response, const string custom_headers = NULL);
    int GET(const string address, uchar & result, string & response, const string custom_headers = NULL);
    int POST(const string address, const string payload, uchar & result, string & response, const string custom_headers = NULL);
    int POST(const string address, const uchar & payload, uchar & result, string & response, const string custom_headers = NULL);
    int request(const string method, const string address, string headers, const uchar & data, uchar & result, string & response);
protected:
...
class Buffer {
public:
    Buffer(int i, const double empty = EMPTY_VALUE, const ENUM_INDEXBUFFER_TYPE type = INDICATOR_DATA, const bool asSeries = false);
    virtual ~Buffer();
...
public:
    BufferArray();
    void add(const int m = 1, const ENUM_INDEXBUFFER_TYPE type = INDICATOR_DATA, const double empty = EMPTY_VALUE, const bool asSeries = false);
    BufferArray(const int n, const bool asSeries = false, const double empty = EMPTY_VALUE);
    BufferArray(const int n, const ENUM_INDEXBUFFER_TYPE & types, const double empty = EMPTY_VALUE, const bool asSeries = false);
    virtual ~BufferArray();
    int size() const;
...
public:
    int dimension;
    string about;
    HoughInfo(const int n, const string s);
    HoughInfo(const HoughInfo & other);
};
...
public:
    virtual int extract(const HoughImage<T>& image, double & result, const int elements = 8);
};

struct SymbolLotExposureRisk {
public:
    double lot;
    int atrPointsNormalized;
    double atrValue;
    double lotFromExposureRaw;
    double lotFromExposure;
    double lotFromRiskOfStopLossRaw;
    double lotFromRiskOfStopLoss;
    double exposureFromLot;
    double marginLevelFromLot;
    int lotDigits;
};
...
public:
    ...
    ulong buy(const double lot, const double p = 0, const double stop = 0, const double take = 0);
    ulong sell(const double lot, const double p = 0, const double stop = 0, const double take = 0);
    ulong buy(const string name, const double lot, const double p = 0, const double stop = 0, const double take = 0);
    ulong sell(const string name, const double lot, const double p = 0, const double stop = 0, const double take = 0);
    ...
    ulong buyStop(const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong sellStop(const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong buyStop(const string name, const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong sellStop(const string name, const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong buyLimit(const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong sellLimit(const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong buyLimit(const string name, const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong sellLimit(const string name, const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong buyStopLimit(const double lot, const double p, const double origin, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong sellStopLimit(const double lot, const double p, const double origin, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong buyStopLimit(const string name, const double lot, const double p, const double origin, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    ulong sellStopLimit(const string name, const double lot, const double p, const double origin, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0);
    bool modify(const ulong ticket, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0, const double origin = 0);
    bool remove(const ulong ticket);
    ulong _pending(const string name, const double lot, const double p, const double stop = 0, const double take = 0, ENUM_ORDER_TYPE_TIME duration = -1, datetime until = 0, const double origin = 0);
    ulong _market(const string name, const double lot, const double p = 0, const double stop = 0, const double take = 0);
};
...
public:
    MultiSymbolMonitor();
    MultiSymbolMonitor(const ENUM_TIMEFRAMES p);
...
public:
    SingleTimer(const int m, const bool paused = false);
    ~SingleTimer();
    virtual int getInterval();
    virtual void stop();
    virtual void start();
...
public:
    ObjectSelector(const string _id, const long _chart = 0);
    virtual string name();
...
public:
    ObjectMonitorBase(const string _id, const int & flags);
    virtual int snapshot();
    virtual void applyChanges(ObjectMonitorInterface * intf);
    virtual void print();
    virtual int backup();
    virtual void restore();
protected:
...
public:
    ObjectMonitor(const string objid, const int & flags);
    virtual int snapshot();
    virtual void print();
    virtual int backup();
    virtual void restore();
    virtual string name();
    virtual void name(const string objid);
    virtual void applyChanges(ObjectMonitorInterface * intf);
};
...
public:
    ...
    uint parameters_total;
    uint opt_params_size;
    uint opt_params_total;
    uint dwords_cnt;
    uint snapshot_size;
    uint passes_total;
    uint passes_passed;
...
public:
    ...
    double equitymin;
    double maxdrawdown_e;
    double drawdownpercent_e;
    double reldrawdown_e;
    double reldrawdownpercnt_e;
    double expected_payoff;
    double profit_factor;
    double recovery_factor;
    double sharpe_ratio;
    double margin_level;
    double custom_fitness;
};
...
public:
    ...
    virtual bool read(const int handle, const int opt, const int gen);
    virtual void print() const;
    virtual void optimized(uchar & buffer, const int i) const;
    virtual string CSVheader() const;
    virtual string CSVline(const int i) const;
};
...
public:
    ...
    virtual string CSVheader() const;
    virtual string CSVline(const int i) const;
    virtual void optimized(uchar & buffer, const int i = -1) const;
};
...
public:
    virtual string stringify(const long v, const ENUM_ORDER_PROPERTY_INTEGER property) const;
    virtual string stringify(const ENUM_ORDER_PROPERTY_INTEGER property) const;
    virtual string stringify(const ENUM_ORDER_PROPERTY_DOUBLE property, const string format = NULL) const;
    virtual string stringify(const ENUM_ORDER_PROPERTY_STRING property) const;
    virtual string stringify(const int i) const;
};

class ActiveOrderMonitor : public OrderMonitor {
public:
    ActiveOrderMonitor(const ulong t);
    virtual bool refresh();
};

class HistoryOrderMonitor : public OrderMonitor {
public:
    HistoryOrderMonitor(const ulong t);
    virtual bool refresh();
};
...
public:
    ...
    static uint getTradeRestrictionsOnSymbol(const string symbol, datetime now = 0, const ENUM_SYMBOL_TRADE_MODE mode = SYMBOL_TRADE_MODE_FULL);
    static bool isTradeOnSymbolEnabled(const string symbol, const datetime now = 0, const ENUM_SYMBOL_TRADE_MODE mode = SYMBOL_TRADE_MODE_FULL);
    static uint getTradeRestrictionsOnAccount();
    static bool isTradeOnAccountEnabled();
    static uint getTradeRestrictionsOnProgram();
    static uint getTradeRestrictions(const string symbol = NULL, const datetime now = 0, const ENUM_SYMBOL_TRADE_MODE mode = SYMBOL_TRADE_MODE_FULL);
    static bool isTradeEnabled(const string symbol = NULL, const datetime now = 0, const ENUM_SYMBOL_TRADE_MODE mode = SYMBOL_TRADE_MODE_FULL);
...
public:
    uint length;
    ChunkType type;
    Chunk(const uint t, const uint l = 0);
...
struct Image {
public:
    Image(const int w, const int h, uchar & data, const uchar c = PNG_CTYPE_TRUECOLOR, const uchar d = 8);
    void write(const int h);
};

class PositionMonitorInterface : public SUPER {
public:
    virtual string stringify(const long v, const ENUM_POSITION_PROPERTY_INTEGER property) const;
    virtual string stringify(const ENUM_POSITION_PROPERTY_DOUBLE property, const string format = NULL) const;
    virtual string stringify(const ENUM_POSITION_PROPERTY_INTEGER property) const;
    virtual string stringify(const ENUM_POSITION_PROPERTY_STRING property) const;
    virtual string stringify(const int i) const;
};
...
public:
    ...
    Tableau(const string pref, const int rows, const int cols, const int height = TBL_CELL_HEIGHT_AUTO, const int width = TBL_CELL_WIDTH_AUTO, const ENUM_BASE_CORNER c = CORNER_RIGHT_LOWER, const int g = 8, const int fsize = 8, const string font = "Consolas", const string bold = "Arial Black", const int mask = TBL_FLAG_COL_0_HEADER, const color bgc = 0x808080, const uchar bgt = 0xC0);
    ~Tableau();
    int getRows() const;
...
public:
    ...
    JS_TYPE t;
    JS_AFFINITY a;
    JsValue(const string _s);
    JsValue(const JS_TYPE _t = JS_OBJECT);
    int size() const;
...
class Container {
public:
    Container(const int h, const string t = NULL);
    ~Container();
...
public:
    ...
    virtual bool refresh();
    virtual void print() const;
    virtual long get(const I property) const;
    virtual double get(const D property) const;
    virtual string get(const S property) const;
    virtual long get(const int property, const long) const;
    virtual double get(const int property, const double) const;
    virtual string get(const int property, const string) const;
    virtual string stringify(const long v, const I property) const;
...
public:
    ...
    TradeState(const MonitorInterface * ptr = NULL);
    void cache();
    void reset();
};
...
class TradeCache {
public:
    TradeCache(const E id, const int error);
...
protected:
    ...
    virtual ulong get(const int i) const;
};

class TradeGuard {
public:
    void push(const ulong t);
    int check(const ulong ticket);
    ulong unsynced();
    bool waitsync(const uint msc = 1000);
    void pop(ulong t = 0);
    void reset();
};
...
public:
    TradeReportWriter();
    virtual bool addCurve(TradeReport, double & data, datetime & when, const string name, const color clr = clrNONE, const int width = 1);
    virtual bool addCurve(double & data, datetime & when, const string name, const color clr = clrNONE, const int width = 1);
    virtual void render();
protected:
    class DataHolder;
};
...
public:
    TradeBaseState(const ulong t);
    void passthrough(const bool b);
...
public:
    ...
    int digits;
    int lotDigits;
    SymbolMetrics(const string s);
    SymbolMetrics(const int d, const int v);
...
public:
    bool direction;
    TradeDirection(const bool sell = false);
    TradeDirection(const ENUM_ORDER_TYPE type);
...
public:
    TrailingStopByMA(const ulong t, const int period, const int offset = 1, const ENUM_MA_METHOD method = MODE_SMA, const ENUM_APPLIED_PRICE type = PRICE_CLOSE);
    virtual double detectLevel();
};
...
public:
    class StaticCreator;
    virtual int encode(uchar & encoded);
    virtual string getData();
    virtual int getData(uchar & buf);
    virtual WS_FRAME_OPCODE getType();
...
public:
    ...
    bool isMasked();
    bool isCompressed();
    WS_FRAME_OPCODE getType();
    int encode(uchar & encoded);
    bool isReady();
    bool isFinal();
    string getData();
    int getData(uchar & buf);
protected:
    uchar FIN;
    uchar RSV1;
    uchar RSV2;
...
public:
    WebSocketFrameHixie(WS_FRAME_OPCODE deftype = WS_FRAME_OPCODE::WS_TEXT_FRAME);
    int encode(uchar & encoded);
    string getData();
    int getData(uchar & buf);
    WS_FRAME_OPCODE getType();
...
class IWebSocketMessage {
public:
    virtual void getFrames(IWebSocketFrame *& frames);
    virtual bool setString(const string & data);
    virtual string getString();
    virtual bool setData(const uchar & data);
    virtual bool getData(uchar & data);
    virtual bool isFinalised();
    virtual bool takeFrame(IWebSocketFrame * frame);
};

class WebSocketMessageHixie : public IWebSocketMessage {
public:
    WebSocketMessageHixie(IWebSocketFrame * f = NULL);
    WebSocketMessageHixie(const string & text);
    ~WebSocketMessageHixie();
    void getFrames(IWebSocketFrame *& frames);
    bool setString(const string & d);
    string getString();
    bool setData(const uchar &);
    bool getData(uchar &);
    bool isFinalised();
    virtual bool takeFrame(IWebSocketFrame * f);
};

class WebSocketMessage : public IWebSocketMessage {
public:
    WebSocketMessage(const bool deflate = false);
    WebSocketMessage(const uchar & bytes, bool deflate = false);
    WebSocketMessage(const string & text, bool deflate = false);
    ~WebSocketMessage();
    bool setData(const uchar & data);
    bool setString(const string & d);
    bool getData(uchar & data);
    string getString();
    void getFrames(IWebSocketFrame *& _frames);
    bool isFinalised();
    virtual bool takeFrame(IWebSocketFrame * frame);
protected:
    uchar _data;
...
public:
    WebSocketConnection(IWebSocketObserver * client, IWebSocketTransport * trans, const bool compression = false);
...
public:
    WebSocketConnectionHybi(IWebSocketObserver * client, IWebSocketTransport * trans, const bool compression = false);
...
public:
    struct HixieKey;
    WebSocketConnectionHixie(IWebSocketObserver * client, IWebSocketTransport * trans, const bool = false);
