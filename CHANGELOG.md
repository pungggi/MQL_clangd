...
- **UI improvements — Includes & compile status**: 
  - **Shorter `#include` Outline labels**: Include entries now show only the basename (e.g. `LiveLog.mqh`) instead of the full `#include <…>` line, cutting horizontal scrolling in the Outline tree. The full include path is preserved in the item detail (visible on hover/peek).
  - **`#include` Go to Definition**: Ctrl/Cmd-Click (or Go to Definition) on an `#include` path now opens the referenced header. Angle-bracket includes (`<Foo/Bar.mqh>`) resolve against the configured Include directory; quoted includes (`"Bar.mqh"`) resolve relative to the current file first, then the Include directory. Unresolvable paths fall through to clangd.
  - **Compile-result status bar**: A new status-bar item reports the last compile/syntax-check outcome — error count, warning count, target label and MQL version — with an icon and color (red errors, amber warnings, green clean). Click it to focus the Problems panel.
  - **Event-handler hover**: Hovering a predefined MQL5 handler (`OnTick`, `OnInit`, `OnCalculate`, …) in your source shows the canonical signature, a short description, and a link to the online docs — reusing the `MQL_EVENT_HANDLERS` set that already powers the Outline grouping.
  - **Input-block CodeLens + "Copy as .set"**: A CodeLens appears above each input group (and above pre-section inputs) showing the group name and input count. Click it to copy every `input`/`sinput` in the document as a Strategy Tester `.set` file to the clipboard — handy for backtest workflows.
  - **Quick fixes for common warnings**: The MQL Code Action provider now offers fixes beyond the existing MQL181 filter — an **explicit cast** `(target)expr` for general "implicit conversion from 'A' to 'B'" warnings (int→double, double→int, …), and **comment-out / remove** actions for unused variable/declaration warnings from clangd (`-Wunused-*`) and MetaEditor.
  - **LiveLog click-to-source links**: In the live tail view, every `{File:Function:Line}` tag emitted by the `Log*()`/`PrintLive()` macros is now a clickable link that jumps to the emitting source line (the `Print` call site). Resolves `__FILE__` Windows paths under Wine and falls back to basename lookup in the MQL data folder / workspace.
  - **LiveLog level filtering**: New **MQL: Filter Live Log by Level** command toggles which levels (INFO/DEBUG/WARN/ERROR/TRADE) are shown in the tail view. Non-log lines always remain visible, and the buffered history re-renders live when the filter changes.
...
- **CompileListenerService Multiple Charts**: `CompileListenerService.mq5` has been refactored to apply the template to _all_ open charts running the specified EA instead of returning early and only updating the first one it finds.
...
- **On-save unresolved-symbol watcher**: After saving a `.mq4`/`.mq5`/`.mqh`, every call clangd cannot resolve gets a wavy underline, a CodeLens above the line (`💡 Unresolved: <name> — click to fix`), and a `Hint` diagnostic that keeps the Problems panel clean. Click the CodeLens or hit `Ctrl+.` to run **Regenerate MQL configuration** or **Restart clangd**. MQL built-ins, names declared in `Include/*.mqh`, and local symbols are filtered out. A canary probe surfaces a file-level CodeLens when clangd cannot resolve any symbol in the file (missing `compile_commands.json` entry, parse failure, etc.).
