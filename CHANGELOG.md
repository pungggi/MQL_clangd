...
- **UI improvements — Includes & compile status**: shorter `#include` Outline labels; Go to Definition on `#include` paths; compile-result status bar (errors / warnings / target / MQL version); event-handler hover with online-doc link; input-block CodeLens with "Copy as .set"; quick fixes for common warnings (explicit cast, comment-out / remove); LiveLog click-to-source links and level filtering.
- **Grouped Outline tree**: Outline, Breadcrumbs and Go to Symbol now group preprocessor and input clutter into **Properties** / **Includes** / **Imports** / **Macros** / **Inputs** / **Event Handlers** (member counts shown; group range spans children). Includes nested `input group` sections, a separate **Macro Functions** group for `#define NAME(...)`, and an opt-in alphabetical sort via `mql_tools.Outline.SortGroupChildren`.
